...
static rtcomm_state * state_from_fd(struct file * fd)
{
...
static int rtcomm_open(struct inode * inode, struct file * fd)
{
        ...
        if (state_from_fd(fd)->is_busy) {
                return (-EBUSY);
        }
        state_from_fd(fd)->is_busy = true;
        ...
        state_from_fd(fd)->is_spi_locked = true;
...
static int rtcomm_release(struct inode * inode, struct file * fd)
{
        ...
        spi_bus_unlock(g_spi->master);
        ppbuff_term(g_ppbuff);
        state_from_fd(fd)->is_spi_locked = false;
        state_from_fd(fd)->is_isr_init   = false;
        state_from_fd(fd)->is_busy       = false;
                ...
                size_t byte_count, loff_t * off)
{
        ...
        memset(&state_from_fd(fd)->spi_transfer, 0, sizeof(state_from_fd(fd)->spi_transfer));
        state_from_fd(fd)->spi_transfer.rx_buf = storage;
        state_from_fd(fd)->spi_transfer.len    = byte_count;
        
        spi_message_init(&state_from_fd(fd)->spi_message);
        spi_message_add_tail(&state_from_fd(fd)->spi_transfer, &state_from_fd(fd)->spi_message);
        state_from_fd(fd)->spi_message.complete = NULL;
        state_from_fd(fd)->spi_message.context  = NULL;
        spi_sync_locked(g_spi, &state_from_fd(fd)->spi_message);
